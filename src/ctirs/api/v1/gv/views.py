...
def _get_exact_matched_info(package_id):
    ...
    for cache_collection in cache_collections:
        ...
        for cache in caches:
            ...
            if isinstance(cache,ObservableCaches) == True:
...
url_pattern = re.compile('url:value\s*=\s*\'(\S+)\'')
ipv4_pattern = re.compile('ipv4-addr:value\s*=\s*\'(\S+)\'')
domain_name_pattern = re.compile('domain-name:value\s*=\s*\'(\S+)\'')
md5_pattern = re.compile('file:hashes\.\'MD5\'\s*=\s*\'(\S+)\'')
sha1_pattern = re.compile('file:hashes\.\'SHA-1\'\s*=\s*\'(\S+)\'')
sha256_pattern = re.compile('file:hashes\.\'SHA-256\'\s*=\s*\'(\S+)\'')
sha512_pattern = re.compile('file:hashes\.\'SHA-512\'\s*=\s*\'(\S+)\'')
